...
export function useSolver(puzzle: PuzzleDefinition) {
  ...
  const advance = useCallback(() => {
    ...
    worker.current.postMessage({
      type: 'advance',
      count: state.current.speed === 'fast' ? 256 : 1,
  ...
  const schedule = useCallback(() => {
    ...
    const delay =
      state.current.solved || state.current.speed === 'fast' ? 0 : state.current.speed === 'slow' ? 300 : 70
  ...
  const start = useCallback(() => {
    ...
    instance.onmessage = ({ data }: MessageEvent<WorkerReply>) => {
      ...
      if (data.result) {
        ...
        setPhase('finished')
        setLatest(null)
        if (data.result.values) setValues(data.result.values)
        else {
          setValues([...puzzle.givens])
          setKinds({})
        }
    ...
    instance.postMessage({ type: 'start', puzzle } satisfies WorkerRequest)
